...
<p align="center">
  <strong>接住那些一闪而过的奇思妙想，用 AI 引导反思，安全地探索水面之下的自己</strong>
</p>
...
- 🔍 **结构化洞察卡片** — 深层解读 + 11 个心理学框架（含阿德勒目的论）+ 可解释的思维惯性 + 行动建议，温暖且非诊断性
- 🌊 **看见长期模式** — 从多条记录中提炼「暗流」主题与认知模式统计
- ✅ **行动闭环** — 收藏建议进入行动清单，首页持续提醒，完成后洞察卡片同步更新
- 🔒 **本地优先** — 想法与对话默认仅存本机 SQLite，AI 分析经代理转发，不留存
...
| **📝 极速捕捉** | **🪞 AI 追问** | **🔍 洞察卡片** |
| :--- | :--- | :--- |
| 全屏输入 · 语音转文字 · 情境记录（时间 / 天气 / 位置） | 苏格拉底式引导 · AI 自决是否继续 · 最多 3 轮 | 深层解读 · 11 个心理学框架（含阿德勒）· 多视角切换 |
...
| **状态管理** | Riverpod |
| **本地存储** | SQLite（sqflite） |
| **AI** | DeepSeek API · Prompt 分层设计（11 个心理学框架） |
| **AI 代理后端** | 阿里云函数计算（FC），Key 存服务端，客户端零配置 |
| **语音转写** | 阿里云 NLS |
| **情境数据** | Open-Meteo + IP 定位 |
...
| 已完成 ✅ | 规划中 💡 |
| :--- | :--- |
| 极速捕捉（文字 + 语音） | 行动复盘（完成后 AI 生成洞察） |
| AI 苏格拉底式追问（最多 3 轮） | 周 / 月度潜意识洞察报告 |
| 洞察卡片 + 11 个框架多视角切换 | Embedding 语义自动关联 |
| 可解释的思维惯性标签 | Google Drive 云端备份 |
| 行动建议收藏与行动闭环 | Android 桌面小组件 |
| 时间轴回顾 + 情绪标签筛选 | iOS 版本 |
| 暗流识别 + 认知模式统计 | |
| 情境记录（时间 / 天气 / 位置） | |
...
│   ├── shell_tab_provider.dart  # 跨页面底部导航切换
│   └── theme/                   # 配色、装饰、主题
├── data/database/               # SQLite 模型与 CRUD（v5 Schema）
├── features/
│   ├── actions/                 # 行动建议清单
│   ├── capture/                 # 首页捕捉
│   ├── detail/                  # 详情 · 追问 · 洞察卡片
...
- **AI 代理**：AI 分析与语音转写经由阿里云函数计算代理转发至 DeepSeek / 阿里云，后端不留存内容
