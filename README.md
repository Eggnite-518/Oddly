...
<p align="center">
  <strong>接住那些一闪而过的奇怪想法，用 AI 引导反思，在本地安全地探索水面之下的自己</strong>
</p>
...
Oddly 不是又一个 AI 日记，而是一条 **Catch → Trace → Mirror** 闭环：

- 📝 **专捕「奇怪想法」** — 首页即输入框，文字 / 语音极速记录，降低捕捉门槛
- 🪞 **苏格拉底式追问** — AI 不急于给答案，而是好奇地引导你往更深一层走（最多 3 轮）
- 🔍 **结构化洞察卡片** — 深层解读 + 心理学视角 + 行动建议，温暖且非诊断性
...
| **📝 极速捕捉** | **🪞 AI 追问** | **🔍 洞察卡片** |
| :--- | :--- | :--- |
| 全屏输入 · 语音转文字 · 情境记录（时间 / 天气 / 位置） | 苏格拉底式引导提问 · AI 自决是否继续 · 最多 3 轮 | 深层解读 · 心理学框架 · 行动建议 · 多视角切换 |
| **📅 时间轴** | **🏷️ 情绪标签** | **🌊 Mirror 模式识别** |
| 按时间回顾 · 按标签筛选 | AI 自动生成 · 支持手动修改 | 暗流（Currents）· 认知模式（Patterns）统计 |

<details>
<summary><strong>Trace 层交互流程（点击展开）</strong></summary>
...
| 时间轴回顾 + 情绪标签筛选 | Android 桌面小组件 |
| 暗流识别 + 认知模式统计 | Embedding 语义自动关联 |
| 情境记录（时间 / 天气 / 位置） | iOS 版本 |
